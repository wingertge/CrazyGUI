...
import com.octagon.crazygui.util.InventoryUtils;
...
public class ContainerBase<T> extends Container {
    ...
    public Set<EntityPlayer> getPlayers() {
        return ((List<ICrafting>) crafters).stream().filter(crafter -> crafter instanceof EntityPlayerMP).map(crafter -> (EntityPlayerMP) crafter).collect(Collectors.toSet());
    }
